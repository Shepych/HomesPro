        ...
        <div class="setup__section accounts clients">
            ...
            <div class="accounts__list">
                @foreach($customers as $user)
                        <div class="account">
                            <div class="account__num">{{$user[0]}}</div>
                                @if(array_key_exists(1, $user))
                                @foreach($user[1] as $access)
                                <form action="{{route('user.change_access')}}" method="POST" class="project">
                                    @csrf
                                        <label class="project__name">
                                            <input type="text" name="contract_number" value="{{$access[0]}}">
                                            <input type="hidden" name="id" value="{{$access[2]}}">
                                        </label>
                                        <div class="project__access">
                                            <select onchange="this.form.submit()" name="access_type">
                                                <option value="watch" @if($access[1]=='watch')selected="True"@endif>Просмотр</option>
                                                <option value="redact" @if($access[1]=='redact')selected="True"@endif>Редактирование</option>
                                            </select>
