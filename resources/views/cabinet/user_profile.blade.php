...
@section('content')
    <section class="section cabinet_top">
        <div class="container">
            ...
            <div class="section__top">
                <div class="section__title">Профиль</div><a class="btn create_access" href="#">
    ...
    <section class="section setup">
        <div class="container">
            <div class="sidebar">
                <a class="sidebar__item sidebar__item_user active" href="{{route('user.cabinet_profile')}}">
                    <div class="icon">
                ...
                <a class="sidebar__item sidebar__item_projects" href="{{route('user.cabinet_personal_projects_list')}}">
                    <div class="icon">
            ...
            <div class="setup__content">
                <div class="setup__section basic_info">
                    <div class="section__title">Основная информация</div>
                    <form method="POST" action="{{route('user.change_major_info')}}">
                        {{ csrf_field()}}
                        <div class="inputs">
                            <label>
                                <input type="text" name="n_s_p" value="@isset(Auth::User()->name){{Auth::User()->surname}} {{Auth::User()->name}}@endisset @isset(Auth::User()->patronymic){{Auth::User()->patronymic}}@endisset"><span>ФИО</span>
                            </label>
                            <label>
                                <input type="text" name="c_c"value="@isset(Auth::User()->address)<?php $address=json_decode(Auth::User()->address);echo($address['0'].', '.$address['1']);?> @endisset"><span>Город</span>
                            </label>
                        </div>
                        <div class="btns">
                            <button class="btn btn_simple btn_change" type="button"><span>Редактировать</span><svg width="33" height="16" viewBox="0 0 33 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <button class="btn btn_submit" type="submit">
                                Подтвердить<svg width="33" height="16" viewBox="0 0 33 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M1 7c-.552285 0-1 .44772-1 1s.447715 1 1 1V7Zm31.7071 1.7071c.3905-.39052.3905-1.02369 0-1.41421L26.3431.92893c-.3905-.390524-1.0236-.390524-1.4142 0-.3905.39052-.3905 1.02369 0 1.41421L30.5858 8l-5.6569 5.6569c-.3905.3905-.3905 1.0236 0 1.4142.3906.3905 1.0237.3905 1.4142 0l6.364-6.364ZM1 9h31V7H1v2Z" fill="#fff" fill-opacity=".5"/>
                                </svg>
                            </button>
                        </div>
                ...
                <div class="setup__section contact_info">
                    <div class="section__title">Контактная информация</div>
                    <form method="POST" action="{{route('user.change_contact_info')}}">
                        {{ csrf_field() }}
                        <div class="inputs">
                            <label>
                                <input type="email" name="email" value="@isset(Auth::User()->email){{Auth::User()->email}}@endisset"><span>E-mail</span>
                            </label>
                            <label>
                                <input class="phone_mask" name="phone" type="tel" value="@isset(Auth::User()->telephone_number){{Auth::User()->telephone_number}}@endisset"><span>Контактный телефон</span>
                            </label>
                            <label>
                                <input type="text" name="c_s_h" value="@isset($address){{$address['2'].' д. '.$address['3']}}@endisset"><span>Адрес</span>
                            </label>
                        </div>
                        <div class="btns">
                            <button class="btn btn_simple btn_change" type="button"><span>Редактировать</span><svg width="33" height="16" viewBox="0 0 33 16" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <div class="setup__section change_pass">
                    <div class="section__title">Изменение пароля</div>
                    <form method="POST" action="{{route('user.change_guard_info')}}">
                        {{ csrf_field() }}
                        <div class="inputs">
                            <label>
                                <input class="pass_mask" type="password" value="123456789"><span>Старый пароль</span>
                            </label>
                            <label>
                                <input class="pass_mask" type="password" name="password"><span>Новый пароль</span>
                            </label>
                            <div class="access"><span>Доступ</span>
                                <select>
                                    <option>@isset(Auth::User()->status){{Auth::User()->status}}@endisset</option>
                                </select>
                            </div>
                        </div>
                        <div class="btns">
